...
export default class PhoneInput extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      code: props.defaultCode ? undefined : "222",
      number: props.value
        ? props.value
        : props.defaultValue
        ? props.defaultValue
        : "",
      modalVisible: false,
      countryCode: props.defaultCode ? props.defaultCode : "MR",
  ...
  onChangeText = (text) => {
    ...
    const formattedInput = asYouType.input(text);
    this.setState({ number: formattedInput });
    const { onChangeText, onChangeFormattedText } = this.props;
    if (onChangeText) {
      onChangeText(text);
    }
    if (onChangeFormattedText) {
      onChangeFormattedText(asYouType.getNumberValue());
    }
  };
